...
function PANEL:FirstPersonControls()
	...
	if ( self.MouseKey == MOUSE_LEFT ) then
		...
		offest = self.offset or Vector(0,0,15)
		self.vCamPos = (self:GetEntity():GetPos() + offest) - self.aLookAngle:Forward() * 10
		return
	end
	self.fFOV = math.Clamp(self.fFOV + y * 0.5, 60, 80)
end 
...
derma.DefineControl( "SLCraftingModelPanel", "A panel containing a model", PANEL, "SLCraftingPanel" )
