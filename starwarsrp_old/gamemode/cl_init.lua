...
hook.Add( "PostPlayerDraw", "ply.FirstPersonRemoveHead", function( ply )
	if ply != LocalPlayer() then return end
	local wep = ply:GetActiveWeapon()
	if !ply.FirstPerson or ply:InVehicle() then 
		if ply.FixFirstPerson then
			for _, v in pairs( ply.BoneIDTable ) do -- Loop through desired bones
				ply:ManipulateBoneScale( v, Vector( 1, 1, 1 ) )
			end
			ply.FixFirstPerson = false
		end
		return 
	end	
	if !ply.FixFirstPerson then
...
local dist = 50
hook.Add( "CalcView", "ply.CameraModeHooks", function( ply, pos, ang, fov )
	if ( !IsValid( ply ) or !ply:Alive() or ply:InVehicle() or ply:GetViewEntity() != ply ) then return end
  	if LocalPlayer().Char and LocalPlayer().Char.faction == "Staff" then return end
	
	local view = {}
	local val = false
	if LocalPlayer().OverrideViewTime and LocalPlayer().OverrideViewTime > CurTime() then
		val = true
	end
	if !val then
		if (LocalPlayer().ViewTime || 0 ) > CurTime() then
			local ent = LocalPlayer().ViewEntity
			if IsValid(ent) then
				pos = ent:GetPos()
			
				view.origin = pos + Vector(0,0, 45)
				view.angles = Angle(90,90,0) + ent:GetAngles()
				view.fov = fov
	...
	if CharDerma or ply.CharModel then
		return {
			origin = Vector("166.865967 -696.599854 -12250.599609"),
			angles = Angle(0,0,0),
			fov = ply:GetFOV(), 
			drawviewer = true
		}
	end
	if spectate_player then
			if spectate_player == LocalPlayer() then
				spectate_player = nil
			end
		if IsValid(spectate_player) then
			
			if LocalPlayer().Char.faction == "Staff" then -- not sure why I even bother, if you were able to activate this anyway. You should just be able to write the hook for it
				local pos = spectate_player:GetPos()
				local angles = ang
				local view = {}
				
				view.origin = pos-( angles:Forward()*100 ) + Vector(0,0, 45)
				view.angles = angles
				view.fov = fov
				view.drawviewer = true
	...
	if LocalPlayer():GetActiveWeapon() then 
		...
		if !eyes then eyes = {Pos = pos} end
		local angs = ang
		local f = 1
		local r = 0


		if !LocalPlayer().FirstPerson then
			f = FORWARD || 50
			r = RIGHT || 0
		end
		if IsValid(LocalPlayer():GetActiveWeapon()) then
			
			if LocalPlayer():GetActiveWeapon():GetNetworkedBool( "Ironsights", false ) then
				fov = fov/2
		...
		local trace = util.TraceHull( {
			start = pos,
			endpos = pos - ang:Forward() * f + ang:Right() * r,
			filter = { ply:GetActiveWeapon(), ply },
			mins = Vector( -4, -4, -4 ),
			maxs = Vector( 4, 4, 4 ),
		} )

		if ( trace.Hit ) then pos = trace.HitPos else pos = pos - ang:Forward() * f  + ang:Right() * r end
...
function asdasdasd(window )
		window = vgui.Create("DFrame")
		window:MakePopup()
		window:SetSize( math.min( ScrW() - 16, 1000 ), math.min( ScrH() - 16, 700 ) )
		window:Center()
		...
		local butt = vgui.Create("DButton", window)
		butt:SetSize(100,100)
		butt:SetText("Get Bodygroups")
		butt.DoClick = function()
			chat.AddText(Color(200,10, 199), "[PlayerModels] ", color_white,mdl.Entity:GetModel() )
		end
	end

concommand.Add("Boom", function()
	asdasdasd(window)
end)
...
concommand.Add("tes_youtube", function(ply)
	...
	html:Dock( FILL )
--	html:OpenURL("https://www.osirisgaming.co.uk/ShootingRange.mp4")
	html:SetHTML(GetHTMLScript("https://www.youtube.com/watch?v=GlM00dz-Jo0"))
end)