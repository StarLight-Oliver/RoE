RegisterITEMS( {
	name = "Repair Gun", -- name of the item
	weapon = {"Melee", "Repair Gun"}, -- is it a weapon
...
local tbl = {
				...
				if IsValid(self.Owner:GetEyeTrace().Entity) then
					...
					if ent:GetPos():Distance(self.Owner:GetPos()) < 200 then
						if ent.Engineering then
							...
							self.Owner:SLChatMessage({Color(100,100,100),"[Engineering] ", color_white, "This entity has " .. ent.Healt .. "% Health"})
						end
						if ent.LFS then
							self.Weapon:SetNextSecondaryFire( CurTime() + 2 )
			...
			DrawModel = function(self)
				...
				if !self.grapplehook then
					...
					--self.grapplehook:FollowBone(self.Owner,self.Owner:LookupBone("ValveBiped.Bip01_R_Hand"))
